...
const getItemIcon = (itemType: string) => {
  switch (itemType) {
    ...
    case 'poll':
      return <BarChart2 className="w-5 h-5 text-violet-600" />;
    case 'reflection':
      return <PenLine className="w-5 h-5 text-teal-600" />;
    case 'assignment':
    ...
    case 'schedule_picker':
      return <CalendarDays className="w-5 h-5 text-green-600" />;
    default:
...
// Inline interactive: renders inside JourneyInlineViewer, auto-completes on submit
const isInlineInteractive = (itemType: string) =>
  ['poll', 'reflection'].includes(itemType);

// Navigate-out interactive: has its own page, opened by the primary button
const isNavigateOutInteractive = (itemType: string) =>
  ['assignment', 'faq', 'schedule_picker'].includes(itemType);
...
const ITEM_ROUTE_CONFIG: Record<string, { table: string; route: string; paramType: 'id' | 'slug' }> = {
  ...
  assignment: { table: 'assignments', route: '/assignments', paramType: 'id' },
  faq: { table: 'faqs', route: '/faqs', paramType: 'id' },
  schedule_picker: { table: 'schedule_polls', route: '/schedule', paramType: 'id' },
};
  ...
  useEffect(() => {
    if (items.length > 0 && expandedItemIds.size === 0) {
      const firstUncompleted = items.find(
        (item) => (isContentType(item.item_type) || isInlineInteractive(item.item_type)) && !completions[item.id]
      );
      if (firstUncompleted) {
  ...
  const handleCompleteAndContinue = async (itemId: string) => {
    ...
    setExpandedItemIds((prev) => {
      ...
      const nextItem = items.slice(currentIdx + 1).find(
        (i) => (isContentType(i.item_type) || isInlineInteractive(i.item_type)) && !completions[i.id] && i.id !== itemId
      );
      if (nextItem) {
            ...
            return (
              ...
              >
                <CardContent className="p-5">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <div className="flex-shrink-0 flex items-center gap-1.5">
                          {isNavigateOutInteractive(item.item_type) ? (
                            <Button
                              ...
                              >
                                {expandedItemIds.has(item.id) ? (
                                  ...
                                  <>
                                    ...
                                    {isInlineInteractive(item.item_type) ? 'Start' : isActivityType(item.item_type) ? 'Preview' : 'View'}
                                  </>
                                )}
                              </Button>
                              {!isInlineInteractive(item.item_type) && (
                                <Button
                                  variant="ghost"
                                  size="sm"
                                  onClick={() => handleItemClick(item)}
                  ...
                  {/* Inline Content Viewer — not shown for navigate-out interactive types */}
                  {expandedItemIds.has(item.id) && !isNavigateOutInteractive(item.item_type) && (
                    <div>
                      <JourneyInlineViewer
                        ...
                        onComplete={isInlineInteractive(item.item_type) && !isCompleted
                          ? () => handleToggleCompletion(item.id)
                          : undefined}
                      />

                      {/* "Mark Complete & Continue" — only for content types in course player mode */}
